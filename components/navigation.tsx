      ...
      <nav className="flex items-center space-x-3 text-sm font-medium">
        {Object.entries(siteConfig.paths).map(([name, path]) => (
          <Link
            ...
            key={name}
            className={cn(
              buttonVariants({ variant: 'shadowNavi' }),
              pathname?.startsWith(path)
                ? 'text-foreground font-bold bg-teal-400 h-10 py-2 px-4 border-black group w-max border shadow-[4px_4px_0px_0px_rgba(0,0,0,0.9)]'
